import React, { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { HiArrowRight } from "react-icons/hi";
import Button from "../utility/Button";
import { Link } from "react-router";

const BigProjectBanner = () => {
  const [isLoaded, setIsLoaded] = useState(false);

  useEffect(() => {
    setIsLoaded(true);
  }, []);

  const bannerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.3,
        delayChildren: 0.5,
      },
    },
  };

  const slideUpVariants = {
    hidden: { y: 100, opacity: 0, filter: "blur(15px)" },
    visible: { 
      y: 0, 
      opacity: 1, 
      filter: "blur(0px)",
      transition: { duration: 1.8, ease: [0.16, 1, 0.3, 1] } 
    },
  };

  const imageSlideInVariants = {
    hidden: { x: 100, opacity: 0, scale: 0.95 }, 
    visible: { 
      x: 0, 
      opacity: 1, 
      scale: 1,
      transition: { duration: 2.2, ease: [0.16, 1, 0.3, 1], delay: 0.8 } 
    },
  };

  const textClipVariants = {
    hidden: { clipPath: 'inset(0% 100% 0% 0%)', opacity: 0 },
    visible: { 
      clipPath: 'inset(0% 0% 0% 0%)', 
      opacity: 1,
      transition: { duration: 2, ease: [0.16, 1, 0.3, 1], delay: 1.4 } 
    },
  };

  return (
    <section className="relative bg-primary flex items-center justify-center overflow-hidden lg:py-20 py-5 ">
      <AnimatePresence>
        {isLoaded && (
          <motion.div 
            key="wide-image-banner"
            variants={bannerVariants}
            initial="hidden"
            animate="visible"
            className="max-w-[1980px] lg:px-20 px-5 mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center relative z-10"
          >
            <div className="space-y-3 lg:col-span-5 max-w-[700px]">
              <motion.h1 
                variants={slideUpVariants} 
                className="text-3xl lg:text-7xl font-black text-base-200 leading-[1.05] "
              >
                Transforming <br />
                <motion.span 
                  variants={textClipVariants} 
                  className="text-transparent w-full bg-clip-text bg-linear-to-r from-cyan-400 to-blue-500 text-3xl lg:text-7xl"
                >
                  Living Spaces.
                </motion.span>
              </motion.h1>

              <motion.p variants={slideUpVariants} className="text-xl lg:text-2xl text-base-200/50  max-w-[500px]">
                StyleDecor is a high-performance ecosystem designed to bridge the gap between 
                professional decorators and dream homes.
              </motion.p>

              <motion.div variants={slideUpVariants} className="flex flex-wrap gap-5 pt-4">
                <div className="w-64">
                 <Link to={'services'}>
                  <Button>
                    Explore Projects <HiArrowRight />
                  </Button>
                  </Link>
                </div>
              </motion.div>
            </div>

            {/* RIGHT CONTENT: Occupies 7 columns | Increased Width, Decreased Height */}
            <motion.div 
              variants={imageSlideInVariants}
              className="lg:col-span-7 relative flex justify-center lg:justify-end"
            >
              {/* aspect-video makes it wide (16:9) and shorter than before */}
              <div className="relative w-full max-w-[900px] aspect-video rounded-xl overflow-hidden border border-white/10 shadow-2xl group">
                <img 
                  src="https://i.ibb.co.com/7dMnxxy8/spacejoy-Rq-O6kwm4t-ZY-unsplash.jpg" 
                  alt="Spacejoy Wide" 
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                />
                
                {/* Internal Decorative Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-primary/40 via-transparent to-transparent pointer-events-none" />
              </div>

              {/* Decorative Ring - Adjusted to match wider shape */}
              <motion.div 
                animate={{ scale: [1, 1.02, 1], opacity: [0.1, 0.2, 0.1] }}
                transition={{ duration: 6, repeat: Infinity }}
                className="absolute -inset-4 border border-cyan-500/20 rounded-[2.5rem] -z-10 hidden md:block"
              />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
    </section>
  );
};

export default BigProjectBanner;